...
// 		setup_dda(&ray, g);
// 		raycast_dda(&ray, g);
// 		end.x = ray.map.x * g->map.minimap.scale;
// 		end.y = ray.map.y * g->map.minimap.scale;
// 		if (end.x >= 0 && end.y >= 0 && end.x < img->width && end.y < img->height)
// 			ft_draw_line(img, start, end, MINIMAP_COLOR_LINE); // dibuja el rayo desde el jugador
// 		x += MINIMAP_GRANULARITY_LINE; // más fino = más líneas = menos separación
// 	}
// }
